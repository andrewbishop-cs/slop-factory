...
```json
{
  ...
  "actions": {
    "riding": { "use_equipment": true, "description": "actively surfing on the water", "anchor": "Actively surfing: balanced on hydro-boards on rushing neon water, spray flying, board under their feet." },
    "grounded": { "use_equipment": false, "description": "on foot (dialogue, building, walking)", "anchor": "On foot on solid, dry ground — no board under their feet, no rushing water; any board is an inert object, never ridden." }
  },
  "default_action": "grounded",
  "hook_action": "riding",
  "characters": [
    {
      "name": "circuit",
      ...
      "equipment_tokens": "a sensor-studded cyan hydro-board bristling with small fins",
    ...
    },
    {
      ...
      "equipment_tokens": "a stripped-down minimalist neon-orange speed hydro-board",
...
```
> Consistency fields: `style_anchor` (art style) + `world_anchor` (GENERAL world look, every shot) are injected into every image prompt. **Scene variation is series-agnostic data, not code:** each `Scene` carries a `location` (forces a real, frame-filling environment, killing the reference sheet's blank backdrop) and an `action` tag; the bible's **`actions`** map (`tag → {anchor, use_equipment, description}`) defines what each tag means, with `default_action` (fallback) and `hook_action` (the opener's peak action). The engine injects the action's `anchor` and, when `use_equipment`, each present character's **`equipment_tokens`** (action-conditional gear, e.g. a board only when `riding`; always-on gear stays in `appearance_tokens`). Adding a new series = writing its `actions`, never touching `images.py`. Each character's `reference_image` drives FLUX.2 multi-reference conditioning; `lora_trigger` + the series `lora` block wire in the optional trained cast-LoRA. See `images.py` (§9) and the M2 consistency workflow.
